...
const DoctorAPI = {
  getMySchedule: () => API.get("/api/appointments/doctor-schedule"),

  completeAppointment: (apptId, data) =>
    API.put(`/api/appointments/${apptId}/complete`, data),

  createPrescription: (data) => API.post("/api/prescriptions", data),

  orderLabTest: (data) => API.post("/api/lab-reports/order", data),

  createProfile: (data) => API.post("/api/doctors/profile", data),
